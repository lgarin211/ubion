...
export default function PaymentPartialRefundedPage() {
  ...
  useEffect(() => {
    ...
    if (info) {
      ...
      try {
        ...
        history.unshift({ ...parsed, trxid, status: 'partial_refunded', date: new Date().toISOString() });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-blue-50 text-blue-900">
      <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
        <h1 className="text-3xl font-bold mb-4 text-blue-700">Pembayaran Partial Refunded</h1>
        ...
        {paymentInfo ? (
          <>
            ...
            <p className="mb-2">Status: <span className="text-blue-700 font-semibold">Partial Refunded</span></p>
          </>
        ) : (
          <p className="mb-2">Sebagian pembayaran Anda telah direfund.</p>
        )}
        <button className="mt-6 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700" onClick={() => router.push("/")}>Kembali ke Beranda</button>
      </div>
    </div>
  );
}
